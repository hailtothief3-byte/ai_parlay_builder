...
DEFAULT_STD = {
    ...
    "player_kills": 4.5,
    "player_headshots": 3.5,
    "player_kills_assists": 6.5,
    "player_fantasy_score": 8.0,
    "map_winner": 0.45,
    "match_winner": 0.45,
    "team_winner": 0.45,
}
...
def scan_edges(sport_key: str | list[str], is_dfs: bool | None = None) -> pd.DataFrame:
    ...
    for _, row in merged.iterrows():
        ...
        std = row["std_dev"] if pd.notnull(row["std_dev"]) else DEFAULT_STD.get(row["market"], 5.0)
        ...
        if row["best_for"] == "over":
            ...
            line_delta = projection - line
        elif row["best_for"] == "yes":
            model_prob = row.get("over_prob_model")
            line_delta = model_prob - implied if model_prob is not None else None
        else:
            model_prob = row.get("under_prob_model")
            line_delta = model_prob - implied if model_prob is not None else None
    ...
    return pd.DataFrame(rows).sort_values(["edge", "confidence"], ascending=[False, False])
